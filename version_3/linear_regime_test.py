...
def l1ErrorPlot(filename,target):
    ...
    for index,x in enumerate(data.results):
        if sum(abs(data.controlVecs[index]))<0.005:
            toReturn[0]+= [x[2]]
            toReturn[1] += [x[1]]
...
parentFolder = '/home/frees/Dropbox/_UW/CODA/version3/Autotune/attempt_4'
...
for voltageStepSize in logspace(-3,-5,11):

    currentIterationFolder = parentFolder + '/voltageStep_test_'+str(autoTuneIteration)
    ...
    for i,gateNum in enumerate(order):
        dDot.setParam('pot'+str(gateNum),currentVolts[i]+voltageStepSize)
    ...
    for i in range(len(order)+2):
        ...
        if i==len(order)+1:
            for j in range(len(outputVolts)):
                outputVolts[j] += voltageStepSize
...
'''
    ...
    while not foundNextStep:
        for dataIndex,l1 in enumerate(plot[0]):
            if l1<targetL1:
                currentL1 = l1
                paramMat += [array(currentVolts)]
                paramMat[-1]+= voltageChanges[dataIndex]
                lastError = plot[1][dataIndex]
                dataIndecesUsed+= [dataIndex]
                break
        
        for i,gateNum in enumerate(order):
            dDot.setParam('pot'+str(gateNum),paramMat[-1][i])
        ...
        convergenceData[0] += [currentL1]
        density,ts = analyzeDataFile(dataFolder+'/Run_'+str(runNum)+'.txt')
        pointDistance = sum(abs(density[0:2]-target[0:2]))
        pointDistance += 0.5*sum(abs(array(map(log,ts))-array(map(log,target[4:]))))
        normalizedDistance = pointDistance*plot[2][dataIndex]/plot[1][dataIndex]
